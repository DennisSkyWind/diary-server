...
const PORT = 3333;
const DIARY_DIR = '/home/ubuntu/logseq-notes';
const HTML_FILE = path.join(__dirname, 'index.html');
const DASHBOARD_FILE = path.join(__dirname, 'dashboard.html');
const OPENCLAW_DIR = '/home/ubuntu/.openclaw/workspace';
const COPAW_DIR = '/home/ubuntu/.copaw';
...
const server = http.createServer((req, res) => {
  ...
  if (pathname === '/api/backup-baidu' && req.method === 'POST') {
    ...
    req.on('end', () => {
      try {
        ...
        // 调用 bdpan 上传
        const {execSync} = require('child_process');
        const bdpanPath = '/home/ubuntu/.local/bin/bdpan';
  ...
  if (pathname === '/api/post-weibo' && req.method === 'POST') {
    ...
    req.on('end', async () => {
      ...
      try {
        ...
        const output = execSync(`cd /home/ubuntu && /home/ubuntu/venv-copaw/bin/python "${path.join(__dirname, 'post-weibo.py')}" "${safeContent}"`, {
          encoding: 'utf-8',
          timeout: 45000,
          env: { ...process.env, PYTHONPATH: '/home/ubuntu/venv-copaw/lib/python3.12/site-packages' }
        });
  ...
  if (pathname === '/api/data-files' && req.method === 'GET') {
    ...
    // 1. 股票持仓
    const stockHoldings = '/home/ubuntu/.copaw/data/my_stock_holdings.json';
    if (fs.existsSync(stockHoldings)) {
      dataFiles.push({
        name: '股票持仓',
        category: '财务',
        icon: '📊',
        file: 'my_stock_holdings.json',
        path: stockHoldings,
        desc: '当前持仓、盈亏统计'
      });
    }
    ...
    // 2. 股票关注列表
    const stockList = '/home/ubuntu/.copaw/data/stock_list.json';
    if (fs.existsSync(stockList)) {
      dataFiles.push({
        name: '股票关注列表',
        category: '财务',
        icon: '👁️',
        file: 'stock_list.json',
        path: stockList,
        desc: '自选股、关注股票'
      });
    }
    ...
    // 3. 家庭资产数据库
    const familyFund = '/home/ubuntu/.copaw/data/family_fund.db';
    if (fs.existsSync(familyFund)) {
      const stat = fs.statSync(familyFund);
      dataFiles.push({
        name: '家庭资产数据库',
        category: '财务',
        icon: '💰',
        file: 'family_fund.db',
        path: familyFund,
        desc: '家庭资产、预算计划（SQLite）'
      });
    }
    
    // 4. 纪念日数据
    // 4. SQLite 日程数据库
    const scheduleDb = '/home/ubuntu/.openclaw/workspace/data/schedule.db';
    if (fs.existsSync(scheduleDb)) {
      dataFiles.push({
        ...
        file: 'schedule.db',
        path: scheduleDb,
        desc: 'SQLite日程数据库(schedules+special_days)'
      });
    }
    
    // 旧版 JSON 文件（保留兼容）
    const specialDays = '/home/ubuntu/.openclaw/workspace/data/special_days.json';
    if (fs.existsSync(specialDays)) {
      dataFiles.push({
        name: '纪念日数据(旧)',
        category: '日程',
        icon: '🎂',
        file: 'special_days.json',
        path: specialDays,
        desc: '生日、纪念日、节日'
      });
    }
    
    const events = '/home/ubuntu/.openclaw/workspace/data/events.json';
    ...
    // 6. 家庭健康管理数据库
    const healthDb = '/home/ubuntu/.copaw/data/family_health.db';
    if (fs.existsSync(healthDb)) {
      dataFiles.push({
        name: '家庭健康档案',
        category: '健康',
        icon: '❤️',
        file: 'family_health.db',
        path: healthDb,
        desc: '家庭成员健康记录和检查报告'
      });
    }
    ...
    // 7. 提醒队列
    const reminders = '/home/ubuntu/.openclaw/workspace/data/pending_reminders.json';
    if (fs.existsSync(reminders)) {
      dataFiles.push({
        name: '提醒队列',
        category: '日程',
        icon: '⏰',
        file: 'pending_reminders.json',
        path: reminders,
        desc: '待发送提醒列表'
      });
    }
